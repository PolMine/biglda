...
discard <- c(tm::stopwords("de"), capitalize(tm::stopwords("de")))

speeches_li <- corpus("GERMAPARLMINI") %>%
  as.speeches(s_attribute_name = "speaker", s_attribute_date = "date") %>%
  get_token_stream(p_attribute = "word", subset = {!word %in% discard}) %>%
  keep(function(x) length(x) >= 100L) # drop short documents

speeches <- speeches_li %>%
  sapply(stri_c, collapse = "\n") %>%
  discard(function(x) nchar(x) == 0L) # drop empty documents

docnames <- names(speeches)
names(speeches) <- NULL # equivalent to unname() - but without copy
