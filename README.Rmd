...
```{r gensim_benchmark, eval = FALSE, echo = FALSE}
library(reticulate)
gensim <- reticulate::import("gensim")

cores_to_use <- as.integer(parallel::detectCores() - 2L)
...
gensim_model <- gensim$models$ldamulticore$LdaMulticore(
  ...
  per_word_topics = FALSE,
  workers =  cores_to_use # required to be integer
)

run <- sprintf("gensim_%d", cores_to_use)
report[[run]] <- data.frame(
  tool = run,
...
```{r benchmark_stm, eval = FALSE, echo = FALSE}
# Omitted, because it is much, much slower than topicmodels::LDA()

library(stm)

AP <- readCorpus(AssociatedPress, type = "slam")
